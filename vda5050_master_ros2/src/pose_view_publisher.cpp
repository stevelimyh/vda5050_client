...
std::string PoseViewPublisher::agv_id(
...
PoseViewPublisher::ensure_publisher_locked(
  const std::string& manufacturer, const std::string& serial_number)
{
  ...
  const std::string topic = pose_view_topic(manufacturer, serial_number);
  auto pub = node_->create_publisher<vda5050_master_ros2::msg::PoseView>(
    topic, kQosDepth);
  ...
  {
    VDA5050_INFO(
      "[PoseViewPublisher] sanitized ROS 2 segment for AGV {} -> topic {} "
      "(ROS 2 names cannot start with a digit; '_' was prepended)",
      id, topic);
  }
  VDA5050_INFO(
    "[PoseViewPublisher] created publisher for {} on topic {}", id, topic);
